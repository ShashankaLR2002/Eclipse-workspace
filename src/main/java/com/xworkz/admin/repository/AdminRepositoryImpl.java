...
public class AdminRepositoryImpl implements AdminRepository {
    ...
    @Override
    public List<RegistrationEntity> findFilteredRegistration(String search) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        EntityTransaction transaction = entityManager.getTransaction();

        try {
            transaction.begin();
            Query query = entityManager.createNamedQuery("GetRegisteredListForSorting");
            query.setParameter("search", "%" + search + "%");
    ...
    @Override
    public boolean deleteTrainerInfo(int id) {

        EntityManager entityManager = entityManagerFactory.createEntityManager();
        EntityTransaction transaction = entityManager.getTransaction();
        try {
            TrainerinfoEntity trainer = entityManager.find(TrainerinfoEntity.class, id);
    ...
    @Override
    public long getCountofStartTime(String startTime) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            Query query = entityManager.createNamedQuery("getslotEntityCount");
            query.setParameter("startTimings", startTime);

            long count = (long) query.getSingleResult();
            System.out.println("Count for startTimings: " + count);

            return count;
        } catch (Exception e) {
            System.out.println("Error getting count for startTimings: " + startTime);
            e.printStackTrace();
            return 0;
        } finally {
            entityManager.close();
    ...
    public long getCountofendTime(String endTimings) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        try {
            Query query = entityManager.createNamedQuery("getslotendTimeEntityCount");
            query.setParameter("endTimings", endTimings);
            long count = (long) query.getSingleResult();
            System.out.println("Count for endTimings: " + count);
            return count;
        } catch (Exception e) {
            System.out.println("Error getting count for endTimings: " + endTimings);
            e.printStackTrace();
            return 0;
        } finally {
            entityManager.close();
