...
public class AdminServiceImpl implements AdminService {
    ...
    public List<RegistrationEntity> findFilteredRegistration(String search) {
        return adminRepository.findFilteredRegistration(search);
    }

    @Override
    public boolean deletetrainerdetails(int id) {
        return adminRepository.deleteTrainerInfo(id);
    }

    @Override
    public long getCountofStartTime(String startTimings) {
        long count = adminRepository.getCountofStartTime(startTimings);

        if (count > 0) {

            System.out.println("StartTimeExist" + startTimings);
            return count;
