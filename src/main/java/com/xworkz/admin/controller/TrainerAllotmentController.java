...
public class TrainerAllotmentController {
    ...
    public String ondeletetrainerinfo(@RequestParam int id, Model model)
    {

        Boolean delete= adminService.deletetrainerdetails(id);
        if(delete)
        {
            List<SlotTimingsEntity> slots = adminService.getAllslots();
            model.addAttribute("slots", slots);
            List<TrainerinfoEntity> trainerinfolist = adminService.getAlltrainerdetails();
            model.addAttribute("trainerInfoList", trainerinfolist);
            model.addAttribute("successMessagefordelete", "DetailsDeletedSuccessfully");
            return "trainerslots";
        }
        else
        {
            model.addAttribute("errorMessagefordelete", "Failed ");
        }
